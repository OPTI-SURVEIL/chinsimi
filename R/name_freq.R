...
name_freq_table = function(refdata,start = 1, end= 9999){
  ...
  if(!any(class(refdata) == 'data.table')){
...
name_freq = function(n,refdata,start = 1,end = 9999, log = T){
  ...
  if(!any(class(refdata) == 'data.table')) refdata = name_freq_table(refdata, start, end)

  ns = substr(n,start,end)
  ns[nchar(ns)==0] = NA

  res = refdata[match(ns,nm)]

  freqs = res$freq
  names(freqs) = res$nm
  if(log) log(freqs) else freqs
}
